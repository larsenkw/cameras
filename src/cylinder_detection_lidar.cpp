...
private:
    ...
    float max_fixed_y; // m
    float min_fixed_y; // m
    float filter_x_low; // m
    float filter_x_high; // m
    float filter_y_low; // m
    float filter_y_high; // m
...
public:
    ...
    {
        // Load Parameters
        nh_.param<std::string>("os1", camera_name, "os1");
        ...
        // ROS Objects
        std::string point_topic = "/os1_node/points";
        ...
        filter_x_low = 0.100; // m
        filter_x_high = 5.0; // m
        filter_y_low = -1.00; // m
        filter_y_high = 1.00; // m
        filter_z_low = -0.100; // m
        filter_z_high = 0.500; // m
        resolution = 256.0; // pi/" + camera_name + "xels/m, 256 approx. = 1280 pixels / 5 m
    ...
    void pcCallback(const sensor_msgs::PointCloud2 &msg)
    {
        ...
        // Filtering along x
        pcl::PointCloud<PointT>::Ptr scene_cloud_unfiltered2(new pcl::PointCloud<PointT>);
        pcl::PassThrough<PointT> pass; // passthrough filter
        pass.setInputCloud(scene_cloud_unfiltered);
        pass.setFilterFieldName("x");
        pass.setFilterLimits(filter_x_low, filter_x_high);
        pass.filter(*scene_cloud_unfiltered2);

        // Filtering along y
        pcl::PointCloud<PointT>::Ptr scene_cloud_unfiltered3(new pcl::PointCloud<PointT>);
        pass.setInputCloud(scene_cloud_unfiltered2);
        pass.setFilterFieldName("y");
        pass.setFilterLimits(filter_y_low, filter_y_high);
        pass.filter(*scene_cloud_unfiltered3);

        // Try to remove the ground layer (find the minimum z level and remove a few centimeters up)
        pass.setInputCloud(scene_cloud_unfiltered3);
        ...
        // Filter using a acc        pcl::PassThrough<PointT> pass; // passthrough filterumulator threshold values
